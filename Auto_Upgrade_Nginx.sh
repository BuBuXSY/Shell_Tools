...
if [[ "$installed_version" == "$latest_version" ]]; then
    ...
    echo "\e[1;32m你已经拥有最新的Nginx，不需要更新。"

    read -p "\e[1;32m是否要执行强制更新？[Y/n]: " choice
    
    if [[ -z "$choice" || "$choice" == "Y" || "$choice" == "y" ]]; then

    echo "\e[1;32m执行强制更新操作..."
...
if [ $? -eq 0 ]; then
    ...
    echo -e "\e[1;32mNginx已经停止"
    echo -e "cp /usr/sbin/nginx{,.bak}"
    echo -e "\e[1;32m旧的nginx已做备份"
    cp /usr/sbin/nginx{,.bak}
    cp ~/temp/nginx /usr/sbin/nginx
    echo -e "systemctl start nginx"
    systemctl start nginx
    echo -e "\e[1;32m新的nginx已成功安装"
else
    echo
    ...
    echo "\e[1;32m本地安装的Nginx版本：$installed_version"
    echo "\e[1;32m官网最新版本：$latest_version"
    echo "\e[1;31m版本不一致"        
    echo "\e[1;32m执行更新操作..."
