...
if [[ "$installed_version" == "$latest_version" ]]; then
    ...
    echo -e "\e[1;32m你已经拥有最新的Nginx，不需要更新。"

    read -p "是否要执行强制更新？[Y/n]: " choice
    
    if [[ -z "$choice" || "$choice" == "Y" || "$choice" == "y" ]]; then

    echo -e "\e[1;32m执行强制更新操作..."
...
if [ $? -eq 0 ]; then
    ...
    echo -e " 编译耗时：\e[1;31m"$((end_seconds-start_seconds))"\e[0m 秒"
        ...
        echo -e "\e[1;31m取消更新操作."
        exit 0
	fi        
else 
    echo -e "\e[1;32m本地安装的Nginx版本：$installed_version"
    echo -e "\e[1;32m官网最新版本：$latest_version"
    echo -e "\e[1;31m版本不一致"        
    echo -e "\e[1;32m执行更新操作..."
