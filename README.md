...
#### Nginx 自动更新
## 自动化的 Nginx 安装和升级脚本，支持从源代码编译并优化性能，支持选择最新主线版或稳定版，包含有 QUIC、Brotli、OCSP、GEOIP2、KTLS。
```bash
...
## 特性 ✨
- **版本选择**：安装最新主线版或稳定版的 Nginx 🔥
- **依赖管理**：自动安装编译所需的依赖包 ⚙️
- **备份和恢复**：自动备份现有 Nginx 配置 🗂️
- **性能优化**：启用 Brotli 和 GeoIP2 模块 ⚡
- **systemd 集成**：自动配置 Nginx 为 `systemd` 服务 🔧
- **错误处理**：详细的日志和错误诊断 🛠️
